...
class RepositorySecurityTests(unittest.TestCase):
    ...
    def test_human_created_pr_boundary_dispatches_exact_branch_validation(self) -> None:
        self.assertEqual(
        ...
        self.assertNotIn("gh pr create", self.sync_source)
        self.assertNotIn("gh pr comment", self.sync_source)
        self.assertIn("AI_GENERATED_PR_DISCUSSION=PROHIBITED", self.sync_source)
