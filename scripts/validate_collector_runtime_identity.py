#!/usr/bin/env python3
"""Fail closed unless Collector runtime identity is exact and image-bound."""
...
IMAGE = re.compile(
    r"^ghcr\.io/appolon1908-hue/codestra-telemetry-opentelemetry@sha256:([0-9a-f]{64})$"
)
...
def fail(message: str) -> None:
    raise SystemExit(f"COLLECTOR_RUNTIME_IDENTITY=FAIL reason={message}")


def main() -> None:
    image = os.environ.get("CODESTRA_OTELCOL_IMAGE", "")
    ...
    if not image_match:
        fail("image must use the immutable Collector release repository and digest")
