#!/usr/bin/env python3
"""Validate Codestra OpenTelemetry Collector protected source authority."""
...
def validate_sync(source: str, document: dict) -> None:
    ...
    required = (
        ...
        'SYNC_BRANCH="sync/telemetry-upstream-${UPSTREAM_SHA}"',
        'git read-tree --prefix=upstream/ "${UPSTREAM_SHA}^{tree}"',
        '[[ "$REMOTE_SHA" == "$LOCAL_SHA" ]]',
        "gh pr list",
        "Multiple open synchronization pull requests found.",
        "AI_GENERATED_PR_DISCUSSION=PROHIBITED",
        "A human operator must create the pull request",
    ...
    if "gh pr create" in source or "gh pr comment" in source:
        raise ValueError("ai_generated_pull_request_discussion_forbidden")
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"TELEMETRY_SOURCE_SECURITY=FAIL ERROR={error}") from error
    print("TELEMETRY_SOURCE_SECURITY=PASS")
